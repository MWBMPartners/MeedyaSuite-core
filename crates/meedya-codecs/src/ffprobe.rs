...
use meedya_metadata::types::{AudioCodec, ChannelConfig};
...
/// Raw FFprobe JSON output structures.
...
pub fn resolve_codec(info: &FfprobeAudioInfo) -> AudioCodec {
    match info.codec_name.as_str() {
        ...
        "mp3" => AudioCodec::Mp3,
        "pcm_s16le" | "pcm_s24le" | "pcm_s32le" | "pcm_f32le" => AudioCodec::Wav,
        "aac" => {
...
mod tests {
    ...
    fn parse_aac_lc() {
        ...
        assert_eq!(resolve_codec(&info), AudioCodec::AacLc);
    }
    ...
    fn parse_alac() {
        ...
        assert!(resolve_codec(&info).is_lossless());
    }
    ...
    fn parse_eac3_surround() {
        ...
        assert_eq!(resolve_codec(&info), AudioCodec::Eac3);
    }

    #[test]
    fn parse_he_aac() {
        let info = parse_ffprobe_output(SAMPLE_FFPROBE_HE_AAC).unwrap();
        assert_eq!(resolve_codec(&info), AudioCodec::HeAac);
    }
