...
use meedya_metadata::types::{AudioCodec, SpatialType};
...
pub fn parse_mediainfo_output(json_str: &str) -> Option<MediaInfoResult> {
    ...
    let (codec, spatial_type) = classify_codec(&format, additional_features.as_deref(), channels);
...
/// Classify codec and spatial type from MediaInfo format fields.
fn classify_codec(
    format: &str,
    additional_features: Option<&str>,
    _channels: Option<u32>,
) -> (AudioCodec, SpatialType) {
    match format {
        "E-AC-3" => {
        ...
        _ => (AudioCodec::Unknown, SpatialType::Stereo),
    }
}
