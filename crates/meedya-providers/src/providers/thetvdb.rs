...
impl TheTvdbProvider {
    ...
    pub fn with_base_url(api_key: Option<String>, base_url: impl Into<String>) -> Self {
        Self {
            // No provider-specific User-Agent needed here (TheTVDB
            // authenticates via a bearer token); build_client applies the
            // crate's shared default plus its timeout
            // (MeedyaSuite-core#76).
            client: build_client(""),
            base_url: base_url.into(),
            api_key,
            limiter: default_limiter_for("thetvdb"),
        }
    }
